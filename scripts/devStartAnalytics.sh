...
NODE_ENV=development LOG_LEVEL=debug CONFIG_FILE='../config/analyticConfig.bank' HTTP_PORT=3020 node src/index.js > $PROXYROOT/logs/analytics_bank.log 2>&1 &
echo $! > $PROXYROOT/logs/analytics_bank.pid
echo "Started analytics engine for acme bank, pid $!."

NODE_ENV=development LOG_LEVEL=debug CONFIG_FILE='../config/analyticConfig.ins' HTTP_PORT=3021 node src/index.js > $PROXYROOT/logs/analytics_ins.log 2>&1 &
